...
testList = [1,2,3,4,5]
def firstPlusLen(paramList):
    sum = paramList[0] + len(paramList)
    return sum
print(firstPlusLen(testList))
...
def valuesGreaterThanSecond(paramList):
    newList = []
    for i in range(0,len(paramList),1):
        if paramList[i] > paramList[1]:
            newList.append(paramList[i])
    return newList
